...
import { postsApi, roomsApi, Post, Room } from '@/lib/api';
...
const NewsHotScreen = () => {
  ...
  const events: Post[] = eventsQuery.data?.posts || [];
  const news: Post[] = newsQuery.data?.posts || [];
  const rooms: Room[] = roomsQuery.data?.rooms || [];

  const loading =
    (tab === 'coming' ? eventsQuery.isLoading : newsQuery.isLoading || roomsQuery.isLoading) &&
    !eventsQuery.data && !newsQuery.data;
  ...
  return (
    <ScreenContainer>
      ...
      {/* Tabs — gradient as absolute background */}
      <View style={styles.tabRow}>
        <TouchableOpacity
          onPress={() => setTab('coming')}
          activeOpacity={0.85}
          ...
          <AppText
            variant="small"
            bold
            color={tab === 'coming' ? colors.white : colors.textSecondary}
            style={{ marginLeft: 6 }}
            numberOfLines={1}>
            Coming Soon
          </AppText>
        ...
        <TouchableOpacity
          ...
          <AppText
            variant="small"
            bold
            color={tab === 'watching' ? colors.white : colors.textSecondary}
            style={{ marginLeft: 6 }}
            numberOfLines={1}>
            Everyone's Watching
          </AppText>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            refreshing={eventsQuery.isFetching || newsQuery.isFetching || roomsQuery.isFetching}
            onRefresh={onRefresh}
        ...
        {loading ? (
          ...
          events.length === 0 ? (
            <EmptyState
              ...
              subText="Tap + to create the first one."
              onPress={() => navigation.navigate('CreatePost', { kind: 'EVENT' })}
              ctaLabel="Create Event"
            />
          ) : (
            events.map((p) => (
              <EventCard key={p.id} post={p} onPress={() => navigation.navigate('PostDetail', { id: p.id })} />
            ))
          )
        ) : (
          <>
            ...
            {news.length === 0 && rooms.length === 0 && (
              <EmptyState
                text="Nothing hot right now."
                subText="Post some news or create a room."
                onPress={() => navigation.navigate('CreatePost', { kind: 'NEWS' })}
                ctaLabel="Post News"
              />
            )}
          </>
      ...
      {/* Floating + button — gradient as absolute bg */}
      <TouchableOpacity
        style={styles.fab}
        activeOpacity={0.85}
        onPress={() =>
          navigation.navigate('CreatePost', { kind: tab === 'coming' ? 'EVENT' : 'NEWS' })
        }>
        <LinearGradient
          colors={colors.buttonGradient as unknown as string[]}
          start={colors.gradientStartPoint}
...
const EventCard: React.FC<{ post: Post; onPress: () => void }> = ({ post, onPress }) => {
  const date = post.eventAt
    ? new Date(post.eventAt).toLocaleDateString('en-US', {
  ...
  return (
    <TouchableOpacity activeOpacity={0.85} onPress={onPress} style={styles.card}>
      <Image
        source={{ uri: post.coverUrl || 'https://images.unsplash.com/photo-1489599735734-79b4625e5b76?w=800' }}
      ...
      <View style={styles.cardActions}>
        ...
        <View style={{ flexDirection: 'row' }}>
          <View style={styles.iconChip}>
            ...
            <AppText variant="tiny" color={colors.textSecondary} style={{ marginLeft: 4 }}>
              {post.rsvpCount}
            </AppText>
          </View>
      ...
      {post.body ? (
        <AppText variant="small" color={colors.textSecondary} numberOfLines={3}
        ...
        {post.author.avatarUrl ? (
          ...
          <View style={[styles.authorAvatar, styles.authorAvatarFallback]}>
            <AppText variant="tiny" bold>
              {(post.author.fullName || post.author.username).slice(0, 1).toUpperCase()}
            </AppText>
          </View>
        )}
        <AppText variant="small" color={colors.textSecondary} style={{ marginLeft: 6 }}>
...
const LiveRoomCard: React.FC<{ room: Room; onJoin: () => void }> = ({ room, onJoin }) => (
  <TouchableOpacity activeOpacity={0.85} onPress={onJoin} style={styles.card}>
    <View style={styles.cardImgWrap}>
      <Image
        source={{ uri: room.thumbnailUrl || 'https://images.unsplash.com/photo-1489599735734-79b4625e5b76?w=800' }}
        style={styles.cardImg}
      />
    ...
    {room.videoTitle ? (
      <AppText variant="small" color={colors.textSecondary} numberOfLines={1} style={{ marginTop: 4 }}>
        Now playing: {room.videoTitle}
      </AppText>
    ) : null}
  </TouchableOpacity>
);
...
const EmptyState: React.FC<{
  text: string; subText: string; ctaLabel: string; onPress: () => void;
